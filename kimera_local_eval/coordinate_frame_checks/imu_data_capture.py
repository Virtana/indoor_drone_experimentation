...
if __name__ == "__main__":
    ...
    output_dir_path = f"../data/coordinate_frame_checks/{dt_string}/"
    ...
    # Load configuration.
    with open('../kimera_data_generation/config.json') as f:
        config = json.load(f)
        f.close()
                ...
                if message is not None:
                    if stream_name == 'imu':
                        for imu_packet in message.packets:
                            ...
                            gyroscope_datapoint, gyroscope_timestamp, accelerometer_datapoint, accelerometer_timestamp = extract_imu_data(imu_packet)
                            # gyroscope_timestamp = time_delta_to_nano_secs((gyroscope_timestamp + curr_timestamp).timestamp())
                            # accelerometer_timestamp = time_delta_to_nano_secs((accelerometer_timestamp + curr_timestamp).timestamp())

                            # print(message.getTimestampDevice(), accelerometer_timestamp, gyroscope_timestamp)
                        ...
                        if counter % 10 == 0:
                            print("\r", end="")
                            print(f"Approximate number of IMU frames captured: {counter}.", end="")
