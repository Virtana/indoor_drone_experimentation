...
def determine_scaling_factor(camera_calibration_resolution, image_size):
	...
	if (height_scale == width_scale):
		return True, height_scale, width_scale
	else:
		return False, height_scale, width_scale


def extract_adjust_camera_calibration(camera_calibration, image_size):
    camera_calibration_resolution = (camera_calibration['width'], camera_calibration['height'])
    result, height_scaling_factor, width_scaling_factor = determine_scaling_factor(camera_calibration_resolution, image_size)
    ...
    params_dict['resolution'] = [image_size[1], image_size[0]]
    params_dict['camera_model']='pinhole'
    params_dict['intrinsics']= [float(intrinsic_matrix[0][0]) * height_scaling_factor, float(intrinsic_matrix[1][1]) * width_scaling_factor, \
                                float(intrinsic_matrix[0][2]) * height_scaling_factor, float(intrinsic_matrix[1][2]) * width_scaling_factor]
...
if __name__ == "__main__":
    ...
    if data is not None:
        ...
        with open('LeftCameraParams.yaml', 'w') as output_file:
        ...
        with open('RightCameraParams.yaml', 'w') as output_file:
