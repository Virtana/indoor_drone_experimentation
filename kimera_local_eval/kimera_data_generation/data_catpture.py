...
def create_pipeline(hz: int, fps: int, sensor_resolution: str) -> depthai.Pipeline:
    ...
    imu.enableIMUSensor(
        [depthai.IMUSensor.ACCELEROMETER_RAW, depthai.IMUSensor.GYROSCOPE_RAW], hz
    )
...
def extract_imu_data(
    imu_packet: depthai.IMUPacket,
) -> Tuple[List[float], Union[int], List[float], Union[int]]:
...
def setup_output_directory(output_dir_path: str) -> bool:
    if os.path.exists(output_dir_path):
        user_input = input(
            "Output directory contains files. If (Y) this folder will be wiped. If (N) this program will exit and you can backup the folder. Enter (Y/N): "
        )
...
def interpolate_data_corrected(
    df: pd.DataFrame, x_col: str, y_col: str, orig_timestamps: np.array
) -> np.array:
    interpolated_values = []
    ...
    for orig_timestamp in orig_timestamps[1:]:
        ...
        if np.isnan(lower_bound_idx) == False and np.isnan(upper_bound_idx) == False:
            ...
            interpolation_fn = interpolate.interp1d(
                bounds_df[x_col].values, bounds_df[y_col].values, kind="linear"
            )
...
def transform_sensor_readings(df: pd.DataFrame, sensor_transforms: dict) -> None:
    ...
    for sensor_col, sensor_transform in zip(
        sensor_cols, list(sensor_transforms.values())
    ):
...
if __name__ == "__main__":
    ...
    sensor_transforms = {
        "x_trans": float(config["X_AXIS_TRANSFORM"]),
    ...
    with device:
        ...
        device.startPipeline(
            create_pipeline(
                hz=imu_fps, fps=camera_fps, sensor_resolution=sensor_resolution
            )
        )
        ...
        while (num_frames_captured <= max_frames - 1) and (stop_capture == False):
            ...
            imu_message = device.getOutputQueue(
                stream_names[0], maxSize=500, blocking=True
            ).tryGet()
            if imu_message is not None:
                for imu_packet in imu_message.packets:
                    ...
                    (
                        gyroscope_datapoint,
                        gyroscope_time,
                        accelerometer_datapoint,
                        accelerometer_time,
                    ) = extract_imu_data(imu_packet)
                    gyroscope_timestamp = time_delta_to_nano_secs(
                        (gyroscope_time + curr_timestamp).timestamp()
                    )
                    accelerometer_timestamp = time_delta_to_nano_secs(
                        (accelerometer_time + curr_timestamp).timestamp()
                    )
        ...
        gyroscope_df.to_csv(
            f"{output_dir_path}/imu0/gyro_data_pre_transform.csv", index=False
        )
        ...
        accelerometer_df = pd.DataFrame(
            accelerometer_data, columns=accelerometer_columns
        )
        accelerometer_df.to_csv(
            f"{output_dir_path}/imu0/acc_data_pre_transform.csv", index=False
        )
        ...
        for y_col in accelerometer_columns[1:]:
            interpolated_data = interpolate_data_corrected(
                accelerometer_df, x_col, y_col, gyroscope_timestamps
            )
            accelerometer_df[y_col] = interpolated_data
            accelerometer_df[y_col] = accelerometer_df[y_col].apply(
                lambda x: f"{x:.5f}"
            )
        accelerometer_df[x_col] = gyroscope_timestamps[0 : len(interpolated_data)]

        accelerometer_df.to_csv(
            f"{output_dir_path}/imu0/acc_data_post_transform_interpolated.csv",
            index=False,
        )
