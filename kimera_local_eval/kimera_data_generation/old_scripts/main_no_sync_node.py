            ...
            for stream_name in stream_names:
                ...
                if message is not None:
                    if stream_name == 'imu':
                        ...
                        # print(pd.Timedelta(message.getTimestamp()).nanoseconds)
                        # imu_timestamp = time_delta_to_nano_secs((message.getTimestamp() + curr_timestamp).timestamp())
                        imu_timestamp = pd.Timedelta(message.getTimestamp(), unit='ns')
                        ...
                        # print(pd.Timedelta(message.getTimestamp()).nanoseconds)
                        left_cam_timestamp = pd.Timedelta(message.getTimestamp(), unit='ns')
                        # left_cam_timestamp = time_delta_to_nano_secs((message.getTimestamp(depthai.CameraExposureOffset.MIDDLE) + curr_timestamp).timestamp())
